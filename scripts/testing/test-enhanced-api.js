...
/**
 * AgriVision Enhanced API Gateway Test Suite
 * Comprehensive testing for all endpoints and features
 */
...
async function testAuthentication() {
  ...
  // Test login
  const loginData = {
    email: 'farmer@example.com', // Use mock user
  ...
  if (loginResult.success) {
    ...
    authToken = loginResult.data.token; // Use mock user token
    return true;
  } else {
...
async function testWeatherAPI() {
  ...
  if (weatherResult.success && weatherResult.data.current) {
    ...
    logTest('Current Weather', 'FAIL', weatherResult.error);
  }
  ...
  if (alertsResult.success && alertsResult.data.smart_alerts) {
    ...
    logTest('Smart Weather Alerts', 'FAIL', alertsResult.error);
    return false;
  }
}
...
async function testAIFeatures() {
  ...
  // Test crop diagnosis (mock image)
  const mockImageData = {
    crop_type: 'maize',
  ...
  // Since we can't easily test file upload in this script, we'll test the endpoint structure
  const diagnosisResult = await makeRequest('POST', '/api/ai/crop-diagnosis', mockImageData);
  
  // This will fail without image, but we can check if the endpoint exists
  if (diagnosisResult.status === 400 && diagnosisResult.error?.error === 'Image file required') {
    logTest('Crop Diagnosis Endpoint', 'PASS', 'Endpoint exists and validates input');
  } else {
    logTest('Crop Diagnosis Endpoint', 'FAIL', 'Unexpected response');
  }
  ...
  const yieldResult = await makeRequest('POST', '/api/yield/predict', yieldData);
  ...
  if (yieldResult.success && yieldResult.data.predicted_yield) {
    ...
    logTest('Yield Prediction', 'FAIL', yieldResult.error);
    return false;
  }
}
...
async function testMarketIntelligence() {
  ...
  if (marketResult.success && marketResult.data.markets) {
    ...
    logTest('Market Intelligence', 'FAIL', marketResult.error);
  }
  
  // Test price opportunities (requires authentication)
  if (authToken) {
    const opportunitiesResult = await authenticatedRequest('GET', '/api/market/opportunities?crops=maize,beans');
    
    if (opportunitiesResult.success && opportunitiesResult.data.opportunities) {
      logTest('Price Opportunities', 'PASS', `Opportunities: ${opportunitiesResult.data.opportunities.length}`);
      return true;
    } else {
      logTest('Price Opportunities', 'FAIL', opportunitiesResult.error);
      return false;
    }
  } else {
    logTest('Price Opportunities', 'SKIP', 'No auth token available');
    return false;
  }
}
...
async function testSMSFeatures() {
  ...
  if (templatesResult.success && templatesResult.data.templates) {
    ...
    logTest('SMS Templates', 'FAIL', templatesResult.error);
  }
  ...
  if (smsResult.success) {
    ...
    logTest('Send SMS', 'FAIL', smsResult.error);
    return false;
  }
}
...
async function testMpesaIntegration() {
  ...
  if (stkResult.success) {
    ...
    logTest('M-Pesa STK Push', 'FAIL', stkResult.error);
  }
  
  // Test service payment (requires authentication)
  if (authToken) {
    ...
    const serviceResult = await authenticatedRequest('POST', '/api/mpesa/pay-service', servicePaymentData);
    
    if (serviceResult.success) {
      logTest('M-Pesa Service Payment', 'PASS', `Service: ${serviceResult.data.service_type}`);
      return true;
    } else {
      logTest('M-Pesa Service Payment', 'FAIL', serviceResult.error);
      return false;
    }
  } else {
    logTest('M-Pesa Service Payment', 'SKIP', 'No auth token available');
    return false;
  }
}
...
async function testCommunityFeatures() {
  ...
  if (postsResult.success && postsResult.data.posts) {
    ...
    logTest('Community Posts', 'FAIL', postsResult.error);
  }
  ...
  if (authToken) {
    ...
    if (shareResult.success) {
      ...
      logTest('Knowledge Sharing', 'FAIL', shareResult.error);
      return false;
    }
  } else {
...
async function testAnalytics() {
  ...
  if (revenueResult.success && revenueResult.data.total_revenue !== undefined) {
    ...
    logTest('Revenue Analytics', 'FAIL', revenueResult.error);
  }
  ...
  if (sustainabilityResult.success && sustainabilityResult.data.overall_score !== undefined) {
    ...
    logTest('Sustainability Scoring', 'FAIL', sustainabilityResult.error);
    return false;
  }
}
...
async function testCooperativeFeatures() {
  ...
  const createCoopData = {
    cooperative_name: 'Test Farmers Cooperative',
    action: 'create'
  };
  
  const createResult = await authenticatedRequest('POST', '/api/cooperatives/join', createCoopData);
  
  if (createResult.success) {
    logTest('Create Cooperative', 'PASS', `Coop ID: ${createResult.data.cooperative?.id}`);
    
    // Test get cooperative info
    const coopId = 'coop_123'; // Use mock ID
    ...
    if (coopResult.success) {
      ...
      logTest('Get Cooperative Info', 'FAIL', coopResult.error);
      return false;
    }
  } else {
    logTest('Create Cooperative', 'FAIL', createResult.error);
    return false;
  }
}
...
async function testPerformance() {
  ...
  try {
    ...
    if (rateLimitedCount > 0) {
      logTest('Rate Limiting', 'PASS', `${rateLimitedCount} requests rate limited`);
      return true;
    } else {
...
async function runAllTests() {
  log('🌾 AgriVision Enhanced API Gateway Test Suite', 'bold');
  log('================================================', 'blue');
